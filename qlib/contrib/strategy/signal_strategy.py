...
class TopkDropoutStrategy_SMS(BaseSignalStrategy):
    # SMS: sell missing signal. This is based on TopkDropoutStrategy with minimal changes:
    # holdings missing from the current signal are added to the sell list. It also
    # compares adjacent signal universes to explicitly detect instruments that
    # just left the benchmark universe and sell them on the next trading step.
    def __init__(
        self,
        *,
        topk,
    ...
    def generate_trade_decision(self, execute_result=None):
        ...
        cash = current_temp.get_cash()
        current_stock_list = current_temp.get_stock_list()
        missing_signal = pd.Index([code for code in current_stock_list if code not in pred_score.index])
        try:
            prev_pred_start_time, prev_pred_end_time = self.trade_calendar.get_step_time(trade_step, shift=2)
            prev_pred_score = self.signal.get_signal(start_time=prev_pred_start_time, end_time=prev_pred_end_time)
        except (IndexError, KeyError):
            prev_pred_score = None
        if isinstance(prev_pred_score, pd.DataFrame):
            prev_pred_score = prev_pred_score.iloc[:, 0]
        left_signal_universe = (
            prev_pred_score.index.difference(pred_score.index) if prev_pred_score is not None else pd.Index([])
        )
        left_signal_universe = pd.Index([code for code in current_stock_list if code in left_signal_universe])
        current_stock_list_with_signal = [code for code in current_stock_list if code in pred_score.index]
        # last position (sorted by score)
        last = pred_score.reindex(current_stock_list_with_signal).sort_values(ascending=False).index
        # The new stocks today want to buy **at most**
        if self.method_buy == "top":
            today = get_first_n(
                pred_score[~pred_score.index.isin(last)].sort_values(ascending=False).index,
        ...
        sell = missing_signal.union(left_signal_universe).union(pd.Index(sell))
        ...
        # note the current has been changed
        current_stock_list = current_temp.get_stock_list()
        buy = today[: max(self.topk - len(current_stock_list), 0)]
        value = cash * self.risk_degree / len(buy) if len(buy) > 0 else 0
