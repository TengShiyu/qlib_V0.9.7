...
class Run(BaseRun):
    ...
    def download_data(
        ...
        """
        if self.interval == "1d":
            max_end = pd.Timestamp(datetime.datetime.now().strftime("%Y-%m-%d")) + pd.Timedelta(days=1)
            if pd.Timestamp(end) > max_end:
                raise ValueError(f"end_date: {end} is greater than tomorrow.")
    ...
    def update_data_to_bin(
        ...
        stale_ratio: float
            ...
            if stale ratio <= stale_ratio, continue automatically; otherwise re-download automatically.
            default 0.001 (0.1%).
        exists_skip: bool
    ...
    def _is_raw_source_up_to_date(
        ...
        logger.warning(f"{yellow}re-download the update window for all symbols by default.{reset}")
        return False
